...
# Initialize stopwords and lemmatizer for text preprocessing
stop_words = set(stopwords.words('english'))
lemmatizer = WordNetLemmatizer()

class MIANA(nn.Module):
    """
    MIANA: LSTM-based neural network model with attention mechanism
    Used for classifying the validity of reviews.
    """
...
def train_category_model(category, dataset_path):
    """
    Train the MIANA model for a specific category using the dataset at dataset_path.
    Saves the trained Word2Vec and MIANA model for later use.
    """
    ...
    # Initialize and train MIANA model
    model = MIANA()
...
def load_model(category):
    ...
    Returns the loaded model or None if not found or error.
    """
...
def upload():
    """
    API endpoint to upload a dataset CSV file and save it in the datasets folder.
    Requires 'file' and 'category' in the POST request.
    """
...
# Flask API routes for rendering HTML templates
@app.route('/')
def home():
...
def predict():
    """
    API endpoint to predict if a review is fake or not.
    Requires 'review' and 'category' in the JSON POST data.
    Returns the fake review probability and prediction.
    """
    try:
        ...
        # If review is too short or empty, flag as fake
        ...
        # Convert input review to vector
        def review_to_vector(review, model):
            vectors = [model.wv[word] for word in review if word in model.wv]
            return np.mean(vectors, axis=0) if vectors else np.full(300, -1)
...
def train_all_models(cat):
    """
    Train models for all dataset files in the 'datasets' folder that match the given category.
    Only trains the model for the specified category.
    """
...
def train_models():
    """
    API endpoint to start model training for a given category.
    Requires 'category' in the JSON POST data.
    """
    try:
        data = request.get_json()
...
# Run Flask server when executed as main script
